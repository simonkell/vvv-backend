...
{
    private $QUERY_VOLUNTEER_GEOLOCATIONS_BY_USERID = "SELECT Substring(`geopoint`, 1, POSITION(',' IN geopoint)-1) as  LAT, Substring(`Geo Point`, POSITION(',' IN geopoint)+1, LENGHT(`Geo Point`)) as LON,  v.user_id, v.radius, FROM volunteer_profile AS v INNER JOIN postalscodes AS p ON v.postal_code = p.postal_code WHERE users.id=?";
